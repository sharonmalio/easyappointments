...
//         $this->load->view("appointments/book");
//     }
...
//     }
    ...
    public function pay()
    {
       ...
       // if(empty($phone)){echo "Please Enter the phone number in this format: 254722000000";}
        ...
        'PartyB' => $shortcode,
        'PhoneNumber' => $phone,
        'CallBackURL' => 'https://webhook.site/30354ba1-f3a4-4477-b6fd-1ed6b1832e35',
    ...
    public function cancel($appointment_hash)
    {
        try {
            ...
            $records = $this->appointments_model->get_batch([
                'hash' => $appointment_hash
            ]);
            if (count($records) == 0) {
            ...
            if ($appointment['id_google_calendar'] != NULL) {
                try {
                    $google_sync = filter_var($this->providers_model->get_setting('google_sync', $appointment['id_users_provider']), FILTER_VALIDATE_BOOLEAN);
            ...
            try {
                ...
                $send_provider = filter_var($this->providers_model->get_setting('notifications', $provider['id']), FILTER_VALIDATE_BOOLEAN);
     ...
     *            Contains the ID of the appointment to retrieve.
     */
    ...
    public function ajax_get_available_hours()
    {
        ...
        try {
            ...
            $this->output->set_content_type('application/json')->set_output(json_encode($available_hours));
        } catch (Exception $exc) {
            $this->output->set_content_type('application/json')->set_output(json_encode([
                'exceptions' => [
    ...
    public function ajax_register_appointment()
    {
        try {
            ...
            try {
                ...
                if ($google_sync == TRUE) {
                    ...
                    if ($post_data['manage_mode'] === FALSE) {
                        ...
                        $appointment['id_google_calendar'] = $this->appointments_model->get_value('id_google_calendar', $appointment['id']);
            ...
            try {
                ...
                if ($send_customer === TRUE) {
                    $email->sendAppointmentDetails($appointment, $provider, $service, $customer, $company_settings, $customer_title, $customer_message, $customer_link, new Email($customer['email']), new Text($ics_stream));
                }
    ...
    public function ajax_get_unavailable_dates()
    {
        try {
            ...
            $provider_list = ($provider_id === ANY_PROVIDER) ? $this->_search_providers_by_service($service_id) : [
                $provider_id
            ];
            ...
            for ($i = 1; $i <= $number_of_days_in_month; $i ++) {
                ...
                foreach ($provider_list as $curr_provider_id) {
                    ...
                    $empty_periods = $this->_get_provider_available_time_periods($curr_provider_id, $service_id, $current_date->format('Y-m-d'), $exclude_appointments);
                    ...
                    if ($service['attendants_number'] > 1) {
                        $available_hours = $this->_get_multiple_attendants_hours($current_date->format('Y-m-d'), $service, $curr_provider);
                        if (! empty($available_hours))
                            break;
                    }
                }
            ...
            $this->output->set_content_type('application/json')->set_output(json_encode($unavailable_dates));
        } catch (Exception $exc) {
            $this->output->set_content_type('application/json')->set_output(json_encode([
                'exceptions' => [
                    exceptionToJavaScript($exc)
    ...
    protected function _check_datetime_availability()
    {
        ...
        $exclude_appointments = (isset($appointment['id'])) ? [
            $appointment['id']
        ] : [];
        ...
        $available_periods = $this->_get_provider_available_time_periods($appointment['id_users_provider'], $appointment['id_services'], date('Y-m-d', strtotime($appointment['start_datetime'])), $exclude_appointments);
     ...
     *            Date to be checked (MySQL formatted string).
     * @param array $excluded_appointment_ids
     *            Array containing the IDs of the appointments that will not be taken into
     *            consideration when the available time periods are calculated.
     *            
     * @return array Returns an array with the available time periods of the provider.
     */
    protected function _get_provider_available_time_periods($provider_id, $service_id, $selected_date, $excluded_appointment_ids = [])
    {
        $this->load->model('appointments_model');
        $this->load->model('providers_model');
        $this->load->model('services_model');
        ...
        foreach ($available_providers as $provider) {
            foreach ($provider['services'] as $provider_service_id) {
                if ($provider_service_id == $service_id) {
                    ...
                    if ($service['attendants_number'] > 1) {
                        $available_hours = $this->_get_multiple_attendants_hours($selected_date, $service, $provider);
                    }
     ...
     *
     * @param array $empty_periods
     *            Contains the empty periods as generated by the "_get_provider_available_time_periods"
     *            method.
     * @param string $selected_date
     *            The selected date to be search (format )
     * @param int $service_duration
     *            The service duration is required for the hour calculation.
     * @param bool $manage_mode
     *            (optional) Whether we are currently on manage mode (editing an existing appointment).
     * @param string $availabilities_type
     *            Optional ('flexible'), the service availabilities type.
     *            
     * @return array Returns an array with the available hours for the appointment.
     */
    protected function _calculate_available_hours(array $empty_periods, $selected_date, $service_duration, $manage_mode = FALSE, $availabilities_type = 'flexible')
    {
        $this->load->model('settings_model');
     ...
     */
    protected function _get_multiple_attendants_hours($selected_date, $service, $provider)
    {
        $this->load->model('appointments_model');
        $this->load->model('services_model');
        $this->load->model('providers_model');
     ...
     *
     * @param string $selected_date
     *            Selected data (Y-m-d format).
     * @param array $periods
     *            Time periods of the current date.
     * @param array $breaks
     *            Breaks array for the current date.
     *            
     ...
     *            Unavailabilities of the current date.
     *            
